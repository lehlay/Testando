...
typedef struct _rkg_ {
 ...
 char n1[50], n2[50], n4[50], n3[50], n5[50];
 double p[5];
...
void ranking(double pontuacao, char* nome, Rkg placar){
 ...
 rec = fopen("records.txt", "r");
       ...
       fgets(placar.n1,50, rec);
       fgets(placar.n2,50, rec);
       fgets(placar.n3,50, rec);
       fgets(placar.n4,50, rec);
       fgets(placar.n5,50, rec);
        /*
            for (i=0; i<5; i++){
               fgets(tempN,50, rec);
               printf("%s", tempN);
